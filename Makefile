...
# Docker
#
# target: docker-build                   - Build the docker images.
.PHONY: docker-build
docker-build:
	@$(call HELPTEXT,$@)
	docker-compose build

# target: docker-push                    - Push the docker images to Docker cloud.
.PHONY: docker-push
docker-push:
	@$(call HELPTEXT,$@)
	docker-compose push

# target: docker-start        	  - Start all Node containers
.PHONY:  docker-start
docker-start:
	@$(call HELPTEXT,$@)
	docker-compose up -d

# target: docker-stop          - Stop all Node containers
.PHONY:  docker-stop
docker-stop:
	@$(call HELPTEXT,$@)
	docker-compose down

# target: node1          - Start Node version 9 server
.PHONY:  node1
node1:
	@$(call HELPTEXT,$@)
	docker-compose up -d node9

# target: node2          - Start Node version 8 server
.PHONY:  node2
node2:
	@$(call HELPTEXT,$@)
	docker-compose up -d node8

# target: node3          - Start Node version 6 server
.PHONY:  node3
node3:
	@$(call HELPTEXT,$@)
